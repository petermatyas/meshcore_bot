...
async def main():
    ...
    mc = await MeshCore.create_serial("/dev/ttyUSB0", debug=False)
    
    try:
        # Get contacts
        repeater = "HU-VA-ha1mp_433_roho"
        ...
        # Send login request
        #print(f"Logging in to repeater '{repeater}'...")
        ...
        telemetry_event = await mc.wait_for_event(EventType.TELEMETRY_RESPONSE, timeout=15)
        if telemetry_event != None:
